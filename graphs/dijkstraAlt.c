...
struct Graph *createGraph(int V)
{
    ...
    graph->V = V;
    graph->array = (struct List *)malloc((V + 1) * sizeof(struct List));
    for (int i = 1; i <= V; ++i)
...
{
    int dist[V + 1];    // Array to store the shortest distance from src to each vertex
    int visited[V + 1]; // Array to mark visited vertices
    for (int i = 1; i <= V; ++i)
    {
...
int main()
{
    int m;
    scanf("%d", &m);
    struct Graph *graph = createGraph(m); // No need to increase size by 1
