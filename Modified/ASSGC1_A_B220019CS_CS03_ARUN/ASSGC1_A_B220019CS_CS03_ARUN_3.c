...
int comparePatients(struct Patient *patientA, struct Patient *patientB)
{
    if (patientA->priority > patientB->priority)
        return 1;
    else if (patientA->priority < patientB->priority)
        return -1;
    else
    {
        int hourPatientA, minutePatientA, hourPatientB, minutePatientB;
        sscanf(patientA->admitTime, "%d:%d", &hourPatientA, &minutePatientA);
        sscanf(patientB->admitTime, "%d:%d", &hourPatientB, &minutePatientB);

        if (hourPatientA > hourPatientB || (hourPatientA == hourPatientB && minutePatientA > minutePatientB))
            return -1;
        else if (hourPatientA < hourPatientB || (hourPatientA == hourPatientB && minutePatientA < minutePatientB))
            return 1;
        else
            return 0;
    }
