...
node* insert(node* root, int key, int val){
    //inserting the node
    if(root==NULL)return createNode(key,val);
    if(key==root->key){
        root->val = val;
        return root;
    }
    else if(key<root->key)root->left = insert(root->left,key,val);
...
node* lower_bound(node* root, int key){
    ...
    if(root->left!=NULL && root->left->key>=key)return lower_bound(root->left,key);
    return root;
}
