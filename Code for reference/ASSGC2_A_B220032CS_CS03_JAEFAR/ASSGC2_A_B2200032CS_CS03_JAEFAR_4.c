...
int validateRB(node* root, int h, int lh){
    ...
    if(col(root)=='R' && (col(root->right)=='R'|| col(root->left)=='R'))return 0;
    int a = (root->color=='B')?1:0;
...
int validBST(node* root){
    ...
    if(l>=root->val || r<=root->val)return 0;
    l = validBST(root->left); r =validBST(root->right);
    if(l==0 || r==0)return 0;
    return 1;
