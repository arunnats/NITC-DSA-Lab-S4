...
void heightify(node* root){
	if(root==NULL) return ;
	int h = -1;
	if(root->left!=NULL && root->left->height>h) h = root->left->height;
	if(root->right!=NULL && root->right->height>h) h = root->right->height;
	root->height = 1+h;
	return;
}
...
node* insert(node* root, int val, int* lcount, int* rcount){
	...
	if(val< root->val) root->left = insert(root->left,val,lcount,rcount);
	else root->right = insert(root->right,val,lcount,rcount);
	//update the heights
	heightify(root);
	//balance factor
	int bal= bf(root);

	if(bal<2 && bal>-2)return root; 
...
void printPath(node* root, int key){
	...
	if(key<root->val) printPath(root->left,key);
	else printPath(root->right,key);
	return;
